...
| `QUARTZ_PUBLIC=false` | `RemoveDrafts` (all except `draft: true`) | `dist-private/` | Azure Static Web Apps | private (Entra, `reader` role) |
...
2. **The SWA gate must use the custom `reader` role**, never the built-in `authenticated` role. `authenticated` admits any Microsoft/Google account; `reader` is granted only by manual invite. See `spec.md` §7 (AD-4).
